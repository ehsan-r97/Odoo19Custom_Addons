...
function convertGregorianStringToJalali(value) {
    if (!value || typeof value !== "string" || typeof farvardin === "undefined") {
        return value;
    }
    ...
    let result;
    try {
        result = farvardin.gregorianToSolar.call(farvardin, year, month, day);
    } catch {
        return value;
    }
    ...
    const convertedDate = `${result[0]}${separator}${leftPad(result[1], 2)}${separator}${leftPad(result[2], 2)}`;
    return rest.length ? `${convertedDate} ${rest.join(" ")}` : convertedDate;
}
...
export function getFormattedValue(record, fieldName, attrs) {
    ...
    const formatOptions = {
        escape: false,
        data: record.data,
        isPassword: "password" in normalizedAttrs,
        digits: normalizedAttrs.digits ? JSON.parse(normalizedAttrs.digits) : field.digits,
        field: record.fields[fieldName],
    };
    if(field.type == "date" || field.type == "datetime"){
        const formattedValue = record.data[fieldName] !== undefined ? formatter(record.data[fieldName], formatOptions) : "";
        if (!formattedValue){
            return formattedValue;
        }
        return convertGregorianStringToJalali(formattedValue);
    }
